"""
Bull Flag Long: Finviz bull flag pattern. Assumes pattern from screener; uses latest bar per ticker.
...
class BullFlagLong(Strategy):
    name = "bull_flag_long"
    TOP_N = 10  # keep only the top 10 by dollar volume to avoid noise
    urls = [
        "https://finviz.com/screener.ashx?v=111&f=cap_midover,geo_usa,sh_opt_option,sh_price_o30,sh_avgvol_o3000,ta_sma50_pa,ta_pattern_bullflag"
    ]
    ...
    def generate(self, prices: pd.DataFrame, history_days: int) -> pd.DataFrame:
        ...
        latest["Reason"] = "Finviz bull flag pattern; filtered by top dollar volume; GPT to set stop/target"
