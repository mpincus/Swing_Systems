"""
MACD Long: bullish MACD line cross above signal in an uptrend.
Filters:
- Price > 30
- Trend: Close > SMA50
- Trigger: MACD (12-26) crosses above signal (9) today; yesterday MACD <= signal.
...
class MACDLong(Strategy):
    name = "macd_long"
    urls = [
        "https://finviz.com/screener.ashx?v=111&f=cap_midover,geo_usa,sh_opt_option,sh_price_o30,sh_avgvol_o1000,ta_sma200_pa"
    ]
    ...
    def generate(self, prices: pd.DataFrame, history_days: int) -> pd.DataFrame:
        ...
        df["SMA200"] = df.groupby("Ticker")["Close"].transform(lambda s: s.rolling(200, min_periods=200).mean())
        ...
        candidates["Reason"] = "MACD cross above signal in uptrend; GPT to set stop/target"
